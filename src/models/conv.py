...
class Conv(nn.Module):
    def __init__(self, data_shape, hidden_size, target_size):
        super().__init__()
        # blocks = [nn.Conv2d(data_shape, hidden_size[0], 3, 1, 1),
        ...
        blocks = [nn.Conv1d(data_shape[0], hidden_size[0], 3, 1, 1),
                  nn.BatchNorm1d(hidden_size[0]),
                  nn.ReLU(inplace=True),
                  nn.MaxPool1d(2)]
        for i in range(len(hidden_size) - 1):
            blocks.extend([nn.Conv1d(hidden_size[i], hidden_size[i + 1], 3, 1, 1),
                           nn.BatchNorm1d(hidden_size[i + 1]),
                           nn.ReLU(inplace=True),
                           nn.MaxPool1d(2)])
        blocks = blocks[:-1]
        blocks.extend([nn.AdaptiveAvgPool1d(1),
                       nn.Flatten()])
        self.blocks = nn.Sequential(*blocks)
        self.linear = nn.Linear(hidden_size[-1], target_size)
