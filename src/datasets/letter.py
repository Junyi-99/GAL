...
def get_dataset(party_id:int, typ:str, value:str, train: str):
    ...
    fname = ""
    if train == "train":
        fname = f"letter_party4-{party_id}_{typ}_{value}_seed0_train.pkl"
    else:
        fname = f"letter_party4-{party_id}_{typ}_{value}_seed0_test.pkl"
...
class Letter(Dataset):
    ...
    def __init__(self, root, split, typ: str, val:str) -> None:
        ...
        self.parties[0] = get_dataset(0, typ, val, split) # (12000,4)
        self.parties[1] = get_dataset(1, typ, val, split) # (12000,4)
        self.parties[2] = get_dataset(2, typ, val, split) # (12000,4)
        self.parties[3] = get_dataset(3, typ, val, split) # (12000,4)
