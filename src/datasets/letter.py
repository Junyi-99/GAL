...
def get_dataset(party_id:int, typ:str, value:str, train: str, dataseed: str):
    ...
    fname = ""
    if train == "train":
        fname = f"letter_party4-{party_id}_{typ}_{value}_seed{dataseed}_train.pkl"
    else:
        fname = f"letter_party4-{party_id}_{typ}_{value}_seed{dataseed}_test.pkl"
...
class Letter(Dataset):
    ...
    def __init__(self, root, split, typ: str, val:str, dataseed: str) -> None:
        ...
        self.parties[2] = get_dataset(2, typ, val, split, dataseed) # (12000,4)
        self.parties[3] = get_dataset(3, typ, val, split, dataseed) # (12000,4)
        self.partitions = [
