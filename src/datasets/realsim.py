...
def get_dataset(party_id:int, typ:str, value:str, train: str, dataseed: str):
    ...
    fname = ""
    if train == "train":
        fname = f"realsim_party4-{party_id}_{typ}_{value}_seed{dataseed}_train.pkl"
    else:
        fname = f"realsim_party4-{party_id}_{typ}_{value}_seed{dataseed}_test.pkl"
...
class Realsim(Dataset):
    ...
    def __init__(self, root, split, typ: str, val:str, dataseed: str) -> None:
        ...
        self.parties[0] = get_dataset(0, typ, val, split, dataseed)
        self.parties[1] = get_dataset(1, typ, val, split, dataseed)
        self.parties[2] = get_dataset(2, typ, val, split, dataseed)
        self.parties[3] = get_dataset(3, typ, val, split, dataseed)
        self.partitions = [
