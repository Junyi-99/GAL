...
def get_dataset(party_id:int, typ:str, value:str, train: str, dataseed: str):
    ...
    fname = ""
    if train == "train":
        fname = f"radar_party4-{party_id}_{typ}_{value}_seed{dataseed}_train.pkl"
    else:
        fname = f"radar_party4-{party_id}_{typ}_{value}_seed{dataseed}_test.pkl"
    
    dir_path = dirname(dirname(abspath(__file__)))
    sys.path.append(dir_path + "/vertibench")

    return pickle.load(open(f"/data/zhaomin/VertiBench/data/syn/radar/{fname}", "rb"))
...
class Radar(Dataset):
    ...
    def __init__(self, root, split, typ: str, val:str, dataseed: str) -> None:
        ...
        self.parties[0] = get_dataset(0, typ, val, split, dataseed) # (260667, 45)
        self.parties[1] = get_dataset(1, typ, val, split, dataseed) # (260667, 43)
        self.parties[2] = get_dataset(2, typ, val, split, dataseed) # (260667, 43)
        self.parties[3] = get_dataset(3, typ, val, split, dataseed) # (260667, 43)
        self.partitions = [
        ...
        self.target = self.y
        self.target_size = len(torch.unique(self.target)) # 7个类
